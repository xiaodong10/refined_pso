...
def transfer_data_grid(map_csv_path):
    ...
    df.replace([-np.inf], -200, inplace=True) # supposed to be -200
    ...
    df['GridX'], df['GridY'] = zip(*df.apply(lambda row: latitude_longitude_to_grid(row['lat'], row['lon'], base_lat, base_lon), axis=1))
    ...
    reshape_df = grid_group.pivot(index='GridY', columns='GridX', values='Power')
...
def plot_map(grid):
    ...
    plt.imshow(grid, cmap='viridis', vmin = -40, vmax = -120, origin='lower', aspect='auto')  # `origin='lower'` makes the [0,0] index appear at the bottom left corner.
...
def main():
    map_csv_path = './coverage_signal_strengths.csv'
